...
export const setupTestEnvironment = async () => {
  ...
  // Check if Firebase is available by making a test call
  let useFirebase = false;
  try {
    const testUsers = await firebaseService.getUsersByRole('client');
    console.log("Firebase is available, will use Firebase for data storage");
    useFirebase = true;
  } catch (error) {
    console.log("Firebase not available or error, will use localStorage:", error);
    useFirebase = false;
  }
  
  // Save Firebase availability status for the app to reference
  localStorage.setItem('courtwise_use_firebase', JSON.stringify(useFirebase));
  
  // Initialize localStorage collections only if Firebase is not available
  if (!useFirebase) {
